...
core_urine_transformed <- rlog(core_urine_dds, blind=FALSE)
...
### 2017-02-10
topGene = "15318_rplY"
geneCounts <- plotCounts(core_dds, gene=topGene, intgroup=c("MEDIA","PRED_PHYLO", "STRAIN"), returnData=TRUE)
ggplot(geneCounts, aes(x=MEDIA, y=count, color=PRED_PHYLO, group=STRAIN)) +
        scale_y_log10() + geom_point(size=3) + geom_line()


x <- counts(core_DE["15318_rplY",], normalized = TRUE)

norm_core_counts <- counts(core_DE, normalized =TRUE)
write.csv(norm_core_counts, "~/git_repos/HUTI-RNAseq/analysis/DE/2017-01-30-htseq-counts/normalized_counts_core.csv", row.names=TRUE)
