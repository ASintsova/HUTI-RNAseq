...
goDat <- read.csv(file="/Users/annasintsova/git_repos/HUTI-RNAseq/results/differential_expression_analysis/gene_association.ecocyc.csv", row.names=1, stringsAsFactors=F) # 19361
myDat <- read.csv(file="/Users/annasintsova/git_repos/HUTI-RNAseq/results/differential_expression_analysis/media_all_genes_edited.csv", row.names=1, stringsAsFactors=F)

FCok <- which( abs(myDat$log2FoldChange) > 2)    # 494 - after removing those with no gene Symbol
FDRok <- which(myDat$padj < 0.05)           # 1956
DE <- intersect(FDRok,FCok) # 494

geneNam <- as.vector(myDat$Name) # 2653
DEgeneNam <- as.vector(myDat$Name[DE]) #494

#geneNam1 <- unlist(strsplit(geneNam,","))  # 2653
#DEgeneNam1 <- unlist(strsplit(DEgeneNam,","))  # 494

# Normalize the sets ----
geneNam2 <- geneNam[which(geneNam %in% goDat$Symbol)] # 2253
DEgeneNam2 <- DEgeneNam[which(DEgeneNam %in% goDat$Symbol)]  # 426
goDat2 <- goDat[which(goDat$Symbol %in% geneNam2),]   # 13932
...
if(nrow(BP.Fisher.elim.Table) > 0) { 
  ...
  write.csv(bDF, file=paste0(results, "media_pathway_analysis_BP.csv") )
}
# MF -------------------
...
if(nrow(MF.Fisher.elim.Table) > 0) { 
  ...
  write.csv(bDF, paste0(results, "media_pathway_analysis_MF.csv") ) 
}
...
CC.Fisher.elim.Table <- BP.Fisher.elim.Table[, !(names(CC.Fisher.elim.Table) %in% c("Term"))]
...
if(nrow(CC.Fisher.elim.Table) > 0) { 
  ...
  write.csv(bDF, paste0(results, "media_pathway_analysis_CC.csv")) 
}
# -----
