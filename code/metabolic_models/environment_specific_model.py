...
def get_reaction_confidences(model, conf_dict, file_out):
    ...
    with open(file_out + ".csv", "w") as fd:
        for k, v in reaction_conf.items():
            fd.write("{}\t{}\t{}\n".format(k,model.reactions.get_by_id(k).name, v))
...
def runCORDA(model, conf_dict, file_out, metabolite = ""):
    ...
    reaction_conf = get_reaction_confidences(model, conf_dict, file_out)
...
def strain_reaction_confidences(strains, data, output_dir, model_dir):
    today = dt.datetime.today().strftime("%Y-%m-%d")

    for strain in strains:
        model = getModel(strain, model_dir)
        UR, UTI = binNormRPKMs(data, strain)
        uti_r_conf = get_reaction_confidences(model, UTI,
                                              os.path.join(output_dir,
                                                           ("{}_{}_UTI").format(today, strain)))
        ur_r_conf = get_reaction_confidences(model, UR,
                                              os.path.join(output_dir,
                                                           ("{}_{}_UR").format(today, strain)))
...
if __name__ == "__main__":
    ...
    #strains = ["HM3"]
    output_dir = "/Users/annasintsova/git_repos/HUTI-RNAseq/results/metabolic_models/"

    #all_envs, all_models = cordaAnalysis(strains, data, output_dir, model_dir)

    strain_reaction_confidences(strains, data, output_dir, model_dir)
